 ...
 */
import { Either, Try, Success, Failure, TimeoutError, Some, None, coreInternals } from "funfix-core";
...
export class IO {
     ...
     */
    delayResult(delay) {
        return this.transformWith(err => IO.delayedTick(delay).flatMap(() => IO.raise(err)), a => IO.delayedTick(delay).map(() => a));
    }
    /**
     ...
     */
    doOnFinish(f) {
        return this.transformWith(e => f(Some(e)).flatMap(() => IO.raise(e)), a => f(None).map(() => a));
    }
    /**
...
export const IOModule = {
    ...
    // ChainRec
    chainRec: (f, a) => IO.tailRecM(a, a => f(Either.left, Either.right, a))
};
